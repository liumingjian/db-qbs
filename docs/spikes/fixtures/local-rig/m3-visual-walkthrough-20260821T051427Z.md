...
本记录里写的「不代所有者决定要不要把它加回来」，当天已经有答案：
**所有者 2026-08-21 裁定就此判废**——`47a2fed` 那次删除按**有意的收窄**对待，
第一版不提供建表 SQL，目标表仍由用户自己在目标库建好。
判据已在清单里改写成 `N/A（判据已随 ADR-0043 退役）`，编号一个没重编、一行没删。
理由、丢掉了什么、以及重估的触发信号，落在 ADR-0043「所有者 2026-08-21 的两条收尾裁定」。
