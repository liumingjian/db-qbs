#!/usr/bin/env bash
# #13 目标端往返实测。只需要 MySQL，不起 Oracle / client（省掉模拟层那几分钟）。
# 幂等，重复跑安全。
set -euo pipefail
cd "$(dirname "$0")/.."

SQL_FILE=${1:-probes/mysql-roundtrip.sql}
...
echo "==> 跑 $SQL_FILE"
# --force：标度不足这类插入**预期会报错**，报错本身就是结论，不能因此中断整轮。
# --show-warnings：静默舍入只以 warning 形式出现，必须打出来。
