...
2. **`vault.centos.org` 前面那层 CDN 开始对 `*.sqlite.bz2` 回 403**（`*.xml.gz` 照常 200），
   ...
   **留一笔账**：`packaging/centos7/Dockerfile` 那份换法还是单源，下次构建镜像会撞上同一个 403。
   ...
   > **后记（当晚 04:5x）**：这笔账当天就清了，实录见
   > [`packaging/centos7/records/build-20260820T045500Z.md`](../../../../packaging/centos7/records/build-20260820T045500Z.md)。
   > 同一趟复测还发现 **403 已经自己好了**（同一个 URL 回 200，单源构建也重新成功）——
   > 所以上面那句「本来会在 #157 的终局演练上炸」说重了：它是一次持续几个小时的抽风，
   > 不是永久下线。后备镜像的理由随之改成**它会自己好、也会自己再坏**，
   > 而装机当天在客户现场引爆没有第二次机会。
