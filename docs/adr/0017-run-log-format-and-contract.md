...
[ADR-0021](0021-m2-log-collection-boundary.md)（**§1、§2、§4、§6.1 由它订正**：采集边界画在主机上、公共字段增 `component`）、
[ADR-0026](0026-m2-in-flight-observation.md)（**§7 的 C2 / C4 两行由它关闭**；第 3 节的 `batch_pushed` 字段是进行中视图全部数字的唯一来源）
...
> **2026-08-15（ADR-0021 §3）复审兑现——载体不变，父子同流。** source 变长驻后 JSON Lines
> **留任主载体**：UI 显示的是历史投影，取证面仍是日志本身。长驻父进程与短命子进程
> **用同一套契约、写同一条 stdout 流**，产生端由第 2 节新增的 `component` 字段分辨。
...
> **2026-08-15（ADR-0021 §3）订正：公共字段由五个变六个**，新增 `component`（见下表末行）。
...
| `component` | string | **产生端**，闭集 `source-orchestrator` / `source-run` / `sink`。**2026-08-15（ADR-0021 §3）新增**：M1 已落盘的旧日志行没有此字段，消费者读到**缺席时一律按 `source-run` 解释**，不得报错、不得当作未知产生端 |
| `task` | string \| null | **子进程收到的任务定义路径**。sink 侧恒为 `null`（它对任务定义一无所知，ADR-0016 §5）。**2026-08-15（ADR-0020 §4）订正**：M1 下它是任务身份；M2 下父进程把定义物化成带 `run_record_id` 的临时 TOML，此字段**不再是任务身份**，任务身份由父进程侧关联。字段与值域不变 |
...
> **2026-08-15（[ADR-0021](0021-m2-log-collection-boundary.md) §1/§2）订正——本条的兜底已被换掉。**
> M2 起图后业务值不再只在 stdout：ADR-0020 §7 把 `column` / `value` 抄进 source 侧 SQLite 历史行，
> ADR-0019 §9 让它经 `/api/*` 送进浏览器。**「值可打」保留原样**（剥离会废掉 #35 的点名要求，
> 不打会废掉 ADR-0013 §7 之后唯一的排障线索），但兜底从**文件权限**换成**主机边界与可达范围**：
>
> - 业务值可存在于 source 服务主机上的**三个载体**——子进程 stdout、部署者重定向出的文件、
>   运行历史 SQLite；三者权限口径仍是 **0600**，但 0600 不再是唯一兜底。
> - **把这三者中任何一个的内容送出该主机，都是外采**，M2 不提供、不支持、不留接口。
> - **可达范围即暴露范围**：M2 不做鉴权，收敛暴露面的唯一手段是 ADR-0021 §4 的 `listen`。
...
- **演进规则**：字段**只增不删、不改义**。~~删字段或改字段含义是破坏性变更，须与 M2 的消费者一并处理。~~
  **2026-08-15（ADR-0020 §2）收紧**：日志已成为运行历史这个用户可见功能的唯一数据来源，
  **删除字段、从闭集删除 `event`、改变字段含义一律禁止**，不存在「与消费者一并处理」这个出口。
  **新增仍然自由**（新 `event`、既有 `event` 的新字段），这是收紧后唯一留着的演进方向。
  理由不对称：新增对既有消费者无害，而删除或改义会让**已落盘的历史行与新日志行语义不同**——
  那不是一次可迁移的变更，是一段无法回溯校正的历史数据。
...
> **2026-08-15（ADR-0021 §1/§6）订正两点。** ①「不得采集到目标端之外」从一条**建议**升格为
> **画在主机上的规格边界**（见第 4 节订正块）；集中式日志与同网段收集器一并被判越界。
> ② 长驻形态下本节多一条 M1 没有的代价：**stdout 无界增长且无轮转**（M1 进程跑完即退，代价有界）。
> **程序仍不建文件、不轮转**——自建轮转是把运维职责搬进产品，写死 journald 则等于默认接受
> 一条被判越界的采集路径。这条作为**明知缺口**记进 `CONTEXT.md`，与凭据明文、日志含值同档。
...
> **2026-08-15 增补（ADR-0020 §8）——`terminal` 一名两义，消费者必须按 `event` 分派。**
> 本闭集中两个 `event` 的 `terminal` 值域**不相交**：`run_finished.terminal` 是 run 终态
> （`SUCCEEDED` / `FAILED`），`commit_diagnosed.terminal` 是 ADR-0012 §5 的 sink 墓碑资源终态
> （`SWAPPED` / `DISCARDED` / `null`）。**程序化消费者不得按字段名合并这两者。**
> 本条只增说明，不改任何既有字段与值域。
...
| 冲突 | 撞上 | M1 的处置 |
|---|---|---|
| C1 历史运行列表、按任务/日期查 | ADR-0012 §2 状态不落盘；墓碑 32 条且非状态存储 | **照实认零**，M1 不引入任何运行历史存储。本 ADR 的第 1、2、5 节把「采集日志重建历史」变成一条**可依赖的可选路线**——M2 在「引入历史存储」与「消费日志」之间选一次。**2026-08-15 已关闭：[ADR-0020](0020-m2-run-history-store.md) 选定「父进程消费子进程日志、落 source 侧同一个 SQLite」，采集用日志、留存用存储，纯「查询时读日志重建」被否** |
| C2 进行中的实时观察 | ADR-0016 §3 source 一次性、无入站接口 | M1 只有 stdout 尾随。**2026-08-15 已关闭：[ADR-0026](0026-m2-in-flight-observation.md) 选定「浏览器 1s 轮询 `GET /api/runs/<run_record_id>`，取 ADR-0019 §4 那份内存投影的当前快照」，SSE 与 WebSocket 均被否（都要求把尽力而为的投影升格成有补发承诺的事件流）；投影只存聚合标量，**UI 不做日志尾随**（§8）** |
| C3 校验失败的取证（差了哪几行） | ADR-0013 暂存表失败即 `DROP`、不留现场 | M1 不留；日志是唯一可能留下线索的地方，而它只留得下第 6 节那些数 |
| C4 进度百分比 | ADR-0007 不预先 `COUNT` | 结构性不能给：总行数未知，且批次不恒为 5000 行。**2026-08-15 复判维持：[ADR-0026](0026-m2-in-flight-observation.md) §3 否掉「买一次 `COUNT(*)`」**——分母落在另一个 SCN、分子是单快照游标累加器，百分比会说谎，且为装饰付 `ORA-01555` 风险。替代是不确定进度条 + 阶段串 + 第 3 节四个数（已推行数／批次序号／已用时／累计字节） |
...
- ~~**第 7 节 C1 在 M2 起图时第一件重开**：选历史存储还是选消费日志，选后者则第 5 节的契约
  ...
  **已兑现（2026-08-15，[ADR-0020](0020-m2-run-history-store.md)）**：采集通道是日志、留存位置是
  source 侧 SQLite；第 5 节的契约已按上文收紧，第 2 节 `task`、第 6.2 节脚注同步订正。
- ~~**第 1 节的载体在 source 变长驻（M2 引入 Web UI）时复审**：届时进度有了推/拉通道，
  stdout 不再是唯一观察面，JSON Lines 是否仍是主载体要重新判。~~
  **已兑现（2026-08-15，[ADR-0021](0021-m2-log-collection-boundary.md) §3）**：**留任主载体**，
  父子两股日志混同一条流，新增公共字段 `component` 分辨产生端。
- ~~**第 4 节的「值可打」绑在「日志文件权限 0600 且不外采」上**。一旦日志要采集到目标端之外
  或接入集中式日志系统，这条必须先重开——那时它就不再是文件权限能兜住的事。~~
  **已兑现（2026-08-15，[ADR-0021](0021-m2-log-collection-boundary.md) §1/§2）**：值已经被采走
  （采集者是父进程、出口是 UI），**值可打保留**，兜底换成主机边界与 `listen` 可达范围。
  新时效：**引入鉴权时整体重开**，与 ADR-0016 §8 一并处理。
  **（2026-08-15 [ADR-0024](0024-m2-unauthenticated-inbound-exposure.md) §1 补：ADR-0021 §2 那句
  「可达范围即暴露范围」只覆盖**读**；覆盖「能做什么」的表述以 ADR-0024 §1 的**权限等价**为准——
  能连上 `source` 端口的人等价于持有该 source 的 Oracle 凭据与目标端写权限。
  本节的「值可打」与 0600 口径一字不改。重开条件见 ADR-0024 §6 三条。）**
