...
本 ADR 定义 **M1 之后**的批次失败处理模型，不改变已经交付的 M1 行为：M1 的 `/v1`
...
commit 和校验失败都没有一份可重放的完整批次，不进本模型，照旧整 run 失败。
...
| 类别 | 判定 | 处置 |
| --- | --- | --- |
| `TRANSIENT` | HTTP 连接/读超时、连接重置、408/429/502/503/504；MySQL 1040/1205/1213 与客户端连接错误 2002/2003/2006/2013 | 当场自动重试；耗尽后物化为失败批次并继续 |
...
source 只分类自己直接观察到的 HTTP 传输错误，不解析中文 `message`。
...
source 在某批第一次命中 `TRANSIENT` 时，先把该批 HTTP JSON body 原始字节持久化，再做两次
自动重试。确认 sink 已提交后删除该文件；三次都失败则保留。目录形状固定为：
...
失败批次号、首次失败时间与过期时间。批次文件就是首次发送的 body，不重新规范化、
不重新查 Oracle。文件与 manifest 都必须用「同目录临时文件 -> `fsync` -> 原子 rename ->
目录 `fsync`」发布；发布失败或磁盘满即 `RUN_FATAL`，不能在没有可靠副本时继续。
任何一次发送得到确认后，都必须先持久化 manifest 中「该批已确认」，再删除批次文件；
崩溃发生在两步之间只会留下一个可清扫的多余文件，不能丢失待重推记录。
...
第二条让不足 20 批的小 run 最多保留 1 个失败批次，较大的 run 最多保留 10%。达到上限时
仍可继续，**超过**才失败。任一断路器触发后立即 abort，已经物化的文件随 run 一起清理，
不再为了凑满一份明显失效的数据继续占着目标端暂存表。

`INCOMPLETE` 的 TTL 固定为 **24 小时**，从第一次进入该状态起算；人工点击不能续期，
也不另设点击次数上限。source 在启动时及运行期间每小时扫描一次：过期后标记
`FAILED(EXPIRED)`、调用幂等 abort，并告警。abort 未确认时保留 manifest 和载荷为
`CLEANUP_PENDING`，下一轮继续清理；确认目标端已丢弃后才删除整个 spool 目录。
...
INCOMPLETE/RETRYING --确定性错误/过期--> FAILED
```
...
sink 重启后可以用 manifest 中原始开 run 元数据**重新挂接**既有暂存表：只允许挂接，
不允许在恢复路径创建或覆盖表；必须重新跑目标元数据与暂存表结构校验，并从 `__batch_no`
重建已收批次集合。暂存表不存在或结构不符即整 run 失败。这样既保留 ADR-0002
