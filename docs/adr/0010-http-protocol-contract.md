...
> **2026-08-13 订正（[ADR-0016](0016-task-definition-form.md) §4，
> [#34](https://github.com/liumingjian/db-qbs/issues/34)）：本小节标题里的「一个判断都不做」
> 作用域写宽了，实为「**逐列类型判定**一个都不做」。**
> 上面那段论证（「一次报全部列」不能降级成「逐端」）只对**需要目标端元数据的逐列比对**成立。
> 而 ADR-0008 预检四条的第 1、2、4 条与 ADR-0009 §3~§4 的「每列显式命名 / 表达式列精度不确定」
> 都要读 **SQL 文本，而 SQL 文本不跨 HTTP**——本 ADR 的报文定稿里没有它，
> **sink 结构性地判不了**，也就不存在「本可以合并却分了两端报」这回事。
> 于是预检明确分两段，各自「一次报全部」：**SQL 形状预检**在 source 本地、
> `POST /runs` **之前**（不过则根本不发请求，sink 不知道存在这个 run，无暂存表、无 `run_id` 泄漏）；
> **映射预检**在 sink，本节其余内容不变。顺序钉死为
> 解析形状 → describe → `POST /runs`：形状不合法时 describe 出的列表不可信。

> **2026-08-15 增补（[ADR-0027](0027-target-ddl-generation-and-cross-end-metadata.md) §6，
> [#57](https://github.com/liumingjian/db-qbs/issues/57)）——一条正交的作用域边界：
> 本节约束的是「判定」，不是「推导」。**
> `source` 从源列元数据**正向推导**目标表建表 SQL（ADR-0009 §2 三条判定式反用，
> 唯一确定）**不落在本节作用域内**：它不比对、不接受、不拒绝任何列，也不需要目标端的任何输入——
> 表还不存在，那边没有东西可查。本节的全部论证（「一次报全部列」不能降级成「逐端」）
> 针对的是**需要目标端元数据的逐列比对**，推导没有目标端这一边，那段论证够不着它。
> **判定权威仍全部在 `sink`，一个字不改**：生成的建表 SQL 不构成任何形式的放行，
> 人按它建完表，映射预检照样在 `POST /runs` 里从头跑一遍、照样可能拒。
> 同一票**否掉**了「新开一条 `source → sink` 目标端元数据查询接口」（ADR-0023 §7 推来的那条），
> **本 ADR 的五个端点与错误码闭集因此一字不动**。
...
| `SWAP_FAILED` | 500 | |
| `SWAP_TARGET_BUSY` | 409 | 2026-08-15 增补，见下（[ADR-0022](0022-concurrent-run-mutual-exclusion.md)） |
...
> **2026-08-13 增补（[ADR-0016](0016-task-definition-form.md) §5，#34）**：那份「sink 的连接配置」
> 定形为 `sink.toml`（MySQL DSN 与凭据、database、监听地址），**与 source 侧的
> `source.toml` / `task-<name>.toml` 三个文件分立、不合并**——正是为了让本节这条论证
> 在文件层同样成立：若让 source 读的任务文件里写着 sink 的 database，
> 权限就又挪回了 source 手上，只是从报文挪到了文件。**sink 全程不读任何任务定义。**
...
- **§9 的 `purged_rows` 保险随 DELETE 一起退役**：写入模型换成主键 upsert 后没有任何行被删，
  该字段**保留但恒为 0**（这句话是真的，不是拿 0 糊弄），「推了 0 行、删了 8 万行」这个报警组合
  在新模型里没有对应的破坏动作。见 [ADR-0035](0035-upsert-write-model.md) §4。
- **错误码闭集不变（15 码）**：ADR-0035 §2 新增的三条主键预检失败复用 `PRECHECK_FAILED`（422），
  只在 `details` 里多一类 issue。
