...
| **轴二 · 资源终态** | ADR-0012 墓碑两值 | **块**：`SWAPPED` 实心（`--ok-bg` 底 + `--ok-bd` 边）、`DISCARDED` 描边中性（**无底色** + 1px `--line-strong` 边） | 回答的是**目标表动没动**，与「为什么」无关。终态块永远带一句中文旁白（`DISCARDED　目标表未被触碰`）。`DISCARDED` 不给底色是硬要求：两个都有底色时，灰度下它们都只是「带边框的填充块」，§8 承诺的「实心块 vs 描边块」就落空了 |
