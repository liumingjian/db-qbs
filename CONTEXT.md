...
> **定位：通用的离线数据库导入方案，不是单客户定制**（2026-08-16 明确，
> [ADR-0005 增补](docs/adr/0005-local-rig-as-v1-verification-baseline.md#台架是通用工具的正当基准不是拿不到客户环境的退路2026-08-16-增补95)）。
> 面向「任意两个网络隔离的 Oracle / MySQL 之间搬数据」。任何一个具体客户环境都是**一个部署**，
> 不是设计基准。**因此：能力边界（类型白名单、映射规则、预检判据）按「类型面 + 目标端往返实测」判，
> 不按某个生产库的命中分布判**；本地 Docker 台架（ADR-0005）是这些判据的正当验证环境，不是退路。
...
　把这条查询实际会吐出来的列拿回来。**不产生运行、不产生 `run_id`、不触碰 `sink`、不写任何存储**，
...
**做**：Oracle → MySQL 单向；SQL 构建器（选表 / 选字段 / **指定日期列** → 生成 SQL，
日期谓词由系统按 ADR-0008 §3 的半开区间形状生成；用户可手改，见 ADR-0023）；
