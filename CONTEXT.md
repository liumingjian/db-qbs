...
　延迟批次重推启用后才增加可持久化的 `INCOMPLETE` / `RETRYING`（见 ADR-0018）。
...
　M1 只用于**顺序断言与诊断**。M1 之后的延迟重推不把序号解释成 Oracle 分页边界，
...
　只保存瞬时错误在三次尝试后仍未确认的批次；不重新查询 Oracle，不保存 checkpoint。
　TTL 为 24 小时，失败面超过断路器则整 run 作废（见 ADR-0018）。
...
　延迟重推启用后会额外加入内部列与索引 `__batch_no`；切换显式排除该列（见 ADR-0018）。
...
　（见 ADR-0017）。`event` 的闭集与写到哪里仍随实现定。
...
业务日期参数化；分批推送（M1 失败即整 run 失败；M1 之后按 ADR-0018 做失败批次延迟重推）；
