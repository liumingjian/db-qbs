...
impl Rig {
    fn open(label: &str) -> Self {
        Self::open_with_key(label, true)
    }

    /// 同一台台架，目标表**没有任何唯一约束**（#261）。
    fn open_without_a_key(label: &str) -> Self {
        Self::open_with_key(label, false)
    }

    fn open_with_key(label: &str, keyed: bool) -> Self {
        ...
        let key_clause = if keyed { ", PRIMARY KEY (`K`)" } else { "" };
        let nullability = if keyed { "NOT NULL" } else { "NULL" };
        plain
            .query_drop(format!(
                "CREATE TABLE `{}`.`{target_table}` (\
             `K` VARCHAR(16) {nullability}, `V` VARCHAR(32) {nullability}{key_clause}\
        ...
        Self {
            ...
            prefix,
            target_table,
            primary_key: if keyed { vec!["K".to_owned()] } else { Vec::new() },
            plain_opts,
            plain,
