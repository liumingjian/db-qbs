...
| # | 东西 | 从哪来 | 装到哪 | 核对方法 |
|---|---|---|---|---|
| 1 | source 完整安装包（`db-qbs-source-linux-amd64-*.tar.gz`） | `packaging/centos7/package.sh` | 源端主机 | 包内 `SHA256SUMS`；含 `db-qbs-source`、`db-qbs-source-run`、Oracle zip 和 `INSTALL.md` |
| 2 | sink 完整安装包（`db-qbs-sink-linux-amd64-*.tar.gz`） | `packaging/centos7/package.sh` | 目标端主机 | 包内 `SHA256SUMS`；含 `db-qbs-sink`、`preflight-target.sh` 和 `INSTALL.md` |
...
| 9 | 包内 `conf/source.toml.example` / `conf/sink.toml.example` | `packaging/customer/`，由 `package.sh` 收入归档 | 两端，安装后填完 `chmod 0600` | source 只写服务字段，**别写已退役的 `sink_base_url`**；sink 不写 `mysql_dsn` / `database`；目标端地址在 source 界面注册 |
| 10 | `INSTALL.md` 与仓库两份安装手册 | `docs/install/source-centos7.md` / `target-centos7.md` | 各自随对应包交付 | 先装 target，再装 source；安装包内手册不依赖仓库 |
